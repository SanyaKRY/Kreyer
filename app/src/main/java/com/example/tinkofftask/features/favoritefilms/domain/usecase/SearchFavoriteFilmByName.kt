...
class SearchFavoriteFilmByName @Inject constructor(
    private val favoriteFilmsScreenRepository: FavoriteFilmsScreenRepository
) {

    fun execute(searchQuery: String): Flow<Result<List<FavoriteFilmDomain>>> {
        return favoriteFilmsScreenRepository.searchFavoriteFilmByName(searchQuery)
    }
}